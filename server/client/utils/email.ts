...
export const sendApprovalEmail = async (requestData: any, credentials: any) => {
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_DGX_API_URL + "/emails/send-approval-email",
      {
        method: "POST",
        headers: {
...
export const sendRejectionEmail = async (requestData: any, remarks: string) => {
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_DGX_API_URL + "/emails/send-rejection-email",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const sendRevokeEmail = async (requestData: any, remarks: string) => {
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_DGX_API_URL + "/emails/send-revoke-email",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const sendGrantAccessEmail = async (requestData: any) => {
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_DGX_API_URL + "/emails/send-grant-access-email",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const sendFunctionalApprovalEmail = async (requestData: any) => {
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_DGX_API_URL +
        "/emails/send-functional-approval-email",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || "Failed to send functional approval email",
      );
    }

    const result = await response.json();
    console.log("Functional approval email sent successfully:", result);
    return result;
  } catch (error) {
    console.error("Error sending functional approval email:", error);
    throw error;
  }
};
