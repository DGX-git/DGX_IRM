...
//       if (userErr) {
//         console.warn("⚠️ No matching user in DB for:", email);
//         setError("Profile not found for this email");
//         return;
//       }
...
//         role_id: dbUser.role_id ? String(dbUser.role_id) : "",
//         contact_no: dbUser.contact_no || "",
//         email_id: dbUser.email_id || "",
//       };

//       setProfileData(plainUser);
//       setEditData(plainUser);

//       // -------------------------------------------
//       // 3️⃣ Fetch role, associations & master data in parallel
//       // -------------------------------------------
//       const [roleResult, assocResult, masterData] = await Promise.all([
//         plainUser.role_id
//           ? supabase
//               .from("role")
//               .select("*")
//               .eq("role_id", plainUser.role_id)
//               .single()
...
//         Promise.all([
//           supabase.from("institute").select("*"),
...
//       // 4️⃣ Set Role Name
//       // -------------------------------------------
//       if (roleResult.data?.role_name) {
//         setDisplayRoleName(roleResult.data.role_name);
//       }

//       // -------------------------------------------
//       // 5️⃣ Extract Master Data
//       // -------------------------------------------
//       const [institutesResult, departmentsResult] = masterData;

//       const institutes =
//         institutesResult.data?.map((item) => ({
//           id: item.institute_id,
//           name: item.institute_name || `Institute ${item.id}`,
//         })) || [];

//       const departments =
//         departmentsResult.data?.map((item) => ({
//           id: item.department_id,
//           name: item.department_name || `Department ${item.id}`,
//           institute_id: item.institute_id,
//         })) || [];

//       setDropdownData({ institutes, departments });

//       // -------------------------------------------
//       // 6️⃣ Associations (Institute + Department Names)
//       // -------------------------------------------
//       if (assocResult.data && assocResult.data.length > 0) {
//         const associations = assocResult.data.map((assoc) => ({
//           id: String(assoc.user_institute_association_id),
//           institute_id: String(assoc.institute_id || ""),
//           department_id: String(assoc.department_id || ""),
//           is_reg_institute: Boolean(assoc.is_reg_institute),
//         }));

//         // Fetch display names in parallel
//         const associationsWithNames = await Promise.all(
//           associations.map(async (assoc) => {
//             const [inst, dept] = await Promise.all([
//               supabase
//                 .from("institute")
//                 .select("*")
//                 .eq("institute_id", assoc.institute_id)
//                 .single(),
//               supabase
//                 .from("department")
//                 .select("*")
//                 .eq("department_id", assoc.department_id)
//                 .single(),
//             ]);

//             return {
//               ...assoc,
//               instituteName: inst.data?.institute_name || "Unknown Institute",
//               departmentName:
//                 dept.data?.department_name || "Unknown Department",
//             };
//           })
//         );
      

//         setInstituteAssociations(associationsWithNames);
//         setEditAssociations(associationsWithNames);
//       }
//     } catch (err) {
...
//         if (changed) {
//           // Check if another association exists with same institute
//           const existingAssoc = instituteAssociations.find(
//             (a) =>
//               !a.is_reg_institute &&
//               a.institute_id === registeredAssoc.institute_id
//           );
...
//                 updated_by: editData.id,
//               })
//               .eq("user_institute_association_id", originalRegisteredAssoc.id);
...
//                 updated_by: editData.id,
//               })
//               .eq("user_institute_association_id", existingAssoc.id);
//           } else {
//             // Normal update
//             await supabase
//               .from("user_institute_association")
...
//                 updated_by: editData.id,
//               })
//               .eq("user_institute_association_id", registeredAssoc.id);
//           }
//         }
//       }
...
//       // if (emailChanged) {
//       //   showSnackbar(
//       //     "Your email has been updated. You will be redirected to the login page. Please log in using your new email."
//       //   );
//       //   setTimeout(() => setShowSuccessSnackbar(false), 8000);
//       // } else {
...
//       setSaving(true);
//       // -------------------------------------
//       // 1️⃣ If email is changed → request Supabase to update it
//       // -------------------------------------
//       if (emailChanged) {
...
//         console.log("✅ Verification code sent to:", editData.email_id);

//         setShowVerificationModal(true);
//       }

//       // If no email change → directly save everything
//       await saveUserData();
//     } catch (error) {
//       console.error("❌ Error updating email or saving:", error);
//       setError("Failed to update email or save profile");
//     } finally {
//       setSaving(false);
//     }
//   };
...
//                               ) : (
//                                 <div className="text-gray-900 font-medium truncate">
//                                   {assoc.instituteName || "Not provided"}
//                                 </div>
//                               )}
//                             </div>
//                           </div>
...
//                               ) : (
//                                 <div className="text-gray-900 font-medium truncate">
//                                   {assoc.departmentName || "Not provided"}
//                                 </div>
//                               )}
//                             </div>
//                           </div>
