...
export default function RegisterForm() {
  ...
  // API call for fetch institutes.
  const fetchInstitutes = async () => {
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_DGX_API_URL + "/register/getInstitutes"
      );
  ...
  // API call for fetch departments.
  const fetchDepartments = async (instituteId: string | number) => {
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_DGX_API_URL +
          `/register/${instituteId}/getDepartments`
      );
      ...
      if (Array.isArray(data)) {
        ...
        // If single object, wrap it in array
        setDepartments([data]);
      } else {
        setDepartments([]);
      }
  ...
  const fetchRoles = async () => {
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_DGX_API_URL + "/register/getRoles"
      );
      const data = await response.json();
      if (data) {
        setRoles(data ?? []);
        ...
        const userRole = data.find(
          (role: RoleItem) => role.role_name?.toLowerCase() === "user"
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_DGX_API_URL}/register/createUser`,
        {
          method: "POST",
    ...
    `}
                  ...
                  >
                    ...
                    {roles
                      .filter(
                        (role: RoleItem) =>
                          role.role_name?.toLowerCase() === "user"
                      )
                      .map((role) => (
