...
//     // Calculate time window (current time to 6 minutes ahead)
//     const timeWindowStart = new Date(now.getTime());
//     const timeWindowEnd = new Date(now.getTime() + 6 * 60 * 1000);
...
// // ✅ Modified query with time-based filtering
...
//         AND uts.selected_date >= :windowStart
//         AND uts.selected_date <= :windowEnd
//       ORDER BY uts.selected_date ASC, ts.time_slot ASC
//       `,
//       {
//         replacements: { 
//           statusId: approvedTechStatus.status_id,
...
//               ramName: req.ram ? `${req.ram} GB` : "", // ✅ added GB,
...
//     // Calculate time window (current time to 7 minutes ahead)
//     const timeWindowStart = new Date(now.getTime());
//     const timeWindowEnd = new Date(now.getTime() + 7 * 60 * 1000);
...
//     const requests = await sequelize.query(
//       `SELECT * FROM fn_get_technical_approval_requests($1, $2, $3)`,
//       {
//         bind: [approvedTechStatus.status_id, timeWindowStart, timeWindowEnd],
//         type: sequelize.QueryTypes.SELECT,
//       }
//     );

//     if (requests.length === 0) {
//       console.log("✅ No pending emails in time window");
//       return;
//     }

//     console.log(
//       `\n📧 CRON JOB RUNNING (Every 1 min) - Checking ${requests.length} approval requests...`
//     );
//     console.log(`📍 Database Timezone: ${DATABASE_TIMEZONE}`);
...
//             const updaterName = `${req.updater_first_name || ""} ${req.updater_last_name || ""}`.trim();
...
cron.schedule("* * * * *", async () => {
  try {
    ...
    if (shouldRefresh) {
      ...
      if (!approvedTechStatus) {
        emailCache = [];
        return;
      }

  // Simple function call - just pass status_id
const requests = await sequelize.query(
  `SELECT * FROM fn_get_technical_approval_requests($1)`,
  {
    bind: [approvedTechStatus.status_id],
    type: sequelize.QueryTypes.SELECT,
  }
);
      ...
      emailCache = Object.values(groupedRequests);
      lastCacheUpdate = now;
    ...
    for (const req of emailCache) {
      try {
        ...
        if (isTimeToSend) {
          ...
          idsToUpdate.push(req.instance_request_id);
          emailsToSend.push({ emailData, credentials });
        }
      } catch (err) {
        console.error(`❌ Error processing request ${req.instance_request_id}:`, err.message);
      }
    }
    ...
    if (idsToUpdate.length > 0) {
      ...
      await sequelize.query(
        `UPDATE instance_request 
         SET technical_approval_email_sent = true, updated_timestamp = NOW()
         WHERE instance_request_id IN (:ids)`,
        {
          replacements: { ids: idsToUpdate },
          type: sequelize.QueryTypes.UPDATE,
        }
      );

      // ✅ Remove sent emails from cache
      emailCache = emailCache.filter(
        req => !idsToUpdate.includes(req.instance_request_id)
      );
    }
    ...
    if (emailsToSend.length > 0) {
      ...
      const emailPromises = emailsToSend.map(async ({ emailData, credentials }) => {
        try {
          await sendApprovalEmailInternal(emailData, credentials);
        } catch (err) {
          console.error(`   ❌ Email failed for ${emailData.instance_request_id}:`, err.message);
        }
      });
...
// ✅ Your existing helper function (no changes)
function formatTimeSlots(timeSlots) {
  if (!timeSlots || timeSlots.length === 0) {
    return "No time slots available";
  }
  ...
  for (let i = 0; i < sortedDates.length; i++) {
    ...
    const times = groupedDetails[date];
    const formattedDate = date.replace(/\//g, "-");
    const formattedTimeRanges = times && times.length > 0 ? formatTimeRanges(times) : "No time slots";
    formattedOutput += `${formattedDate}\n[${formattedTimeRanges}]`;
    if (i < sortedDates.length - 1) formattedOutput += "\n";
  }
